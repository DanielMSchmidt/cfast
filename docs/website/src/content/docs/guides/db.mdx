...
This is application-level Row-Level Security for D1, which has no native RLS. The permission check is structural: you cannot get a query result without going through `.run()`, and `.run()` always checks [`@cfast/permissions`](/guides/permissions/) grants.
