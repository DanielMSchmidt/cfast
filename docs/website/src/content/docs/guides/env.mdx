...
Other cfast packages accept the validated env object directly. This is the primary integration point -- `@cfast/env` feeds typed bindings into [`@cfast/db`](/guides/db/), [`@cfast/auth`](/guides/auth/), and [`@cfast/storage`](/guides/storage/):
...
When using [`@cfast/core`](/guides/core/), the env schema is passed directly to `createApp()` and initialization is handled automatically via `app.init(rawEnv)`.
