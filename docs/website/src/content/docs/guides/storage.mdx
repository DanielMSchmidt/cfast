...
The storage layer handles bytes, not permissions. Gate uploads through [`@cfast/db`](../db/) operations so that permission checks happen before any file is uploaded:
...
### Signed URLs for the proxy route
