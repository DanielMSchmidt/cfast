...
`@cfast/core` is the optional composition layer for the cfast framework. It provides `createApp()` -- a single definition that connects [`@cfast/env`](../env/), [`@cfast/permissions`](../permissions/), [`@cfast/auth`](../auth/), [`@cfast/db`](../db/), [`@cfast/storage`](../storage/), and any other plugins into a typed per-request context on the server and a unified provider tree on the client.
...
### Default Plugin Preset with `createDefaultPlugins()`

Most cfast apps use the same two-plugin chain: auth then db. `createDefaultPlugins()` bundles them into a single call:
...
const { authPlugin, dbPlugin } = createDefaultPlugins({
  initAuth: (env) => initAuth({ d1: env.DB, appUrl: env.APP_URL }),
  createDb: (grants, user) => createDb({ d1: env.DB, schema, grants, user }),
});
...
The auth plugin calls `initAuth(env).createContext(request)` and provides `{ user, grants, instance }` under `ctx.auth`. The db plugin consumes `ctx.auth.grants` and `ctx.auth.user` and provides `{ client }` under `ctx.db`. This replaces the manual `definePlugin` boilerplate for the standard auth-then-db chain.
