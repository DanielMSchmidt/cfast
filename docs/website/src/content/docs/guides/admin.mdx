...
### Simplified Auth Adapter with `createAdminAuthAdapter()`

The `auth` config object passed to `createAdmin` requires six methods (`requireUser`, `hasRole`, `getRoles`, `setRole`, `removeRole`, `setRoles`). Most apps copy-paste ~50 identical lines to delegate each method to the auth instance. `createAdminAuthAdapter()` replaces all of it with a one-liner:
...
The adapter accepts a `getAuth` factory (called on every admin operation for fresh env bindings) and returns a complete `AdminAuthConfig` object. It is equivalent to `createAdminAuth` from `@cfast/auth` -- the only difference is where you import from. Use whichever import is more convenient for your file.
