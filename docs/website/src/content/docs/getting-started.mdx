...
- `compose()` groups multiple operations so they share a permission check and execute together.
        ...
        {loaderData.tasks.map((task) => {
          ...
          return (
            ...
            >
              ...
              {remove.permitted && (
                <ActionButton action={remove} color="danger" variant="soft" size="sm">
                  Delete
                </ActionButton>
              )}
            </li>
...
- **`useActions`** reads the permission metadata from the loader and returns action handles. `remove.permitted` is `false` when the current user isn't the task's assignee — the delete button is hidden automatically.
- **`ActionButton`** is a permission-aware button from `@cfast/ui`. It handles submission, loading state, and confirmation dialogs.

### 5. Add to the Admin Panel

Open `app/admin.server.ts` and register the tasks table in the schema and dashboard:
