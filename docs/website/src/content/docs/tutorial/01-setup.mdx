...
<Aside type="tip">
  `react-router dev` uses the Cloudflare Vite plugin under the hood, so your app runs in the actual Workers runtime -- not a Node.js process. This means any Workers-incompatible code will fail here, not just in production.
</Aside>
